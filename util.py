...
data_path = '/Users/jhussman/Documents/predict-energy/lib/data
...
def clf_labels():
    df = pd.read_csv(data_path + '/output_dispatch.csv', index_col=0)[['LI_chr', 'LI_dis']].copy()
    for c in ['chr', 'dis']:
...
def load_fuel_data():
    f = pd.read_csv(data_path + '/fuel_data-FRSCE1.csv', index_col=0)
...
def parse_fuel_price(previous_day=True):
    df = pd.read_csv(data_path + '/fuel_data.csv', index_col=0)
